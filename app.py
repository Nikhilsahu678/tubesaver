...
def _load_cookies_file():
    """Optional: return a cookies file for yt-dlp.

    Two ways to provide cookies (most reliable fix for YouTube's bot-check):
      1. YTDL_COOKIES_FILE  — path to a Netscape-format cookies.txt file
      2. YTDL_COOKIES       — full cookies.txt content (kept out of the repo;
                              store it in a secret env var on your host)
    Cookies are sensitive — never commit them to a public repo.
    """
    path = os.environ.get("YTDL_COOKIES_FILE")
    if path and os.path.isfile(path):
        return path
    content = os.environ.get("YTDL_COOKIES")
    if content:
        os.makedirs("/tmp/ytdl", exist_ok=True)
        p = "/tmp/ytdl/cookies.txt"
        try:
...
def common_ytdlp_opts():
    """Options shared by info extraction and downloads.

    Uses yt-dlp's default YouTube clients (best quality). If YouTube's
    bot-check blocks them (common on datacenter IPs like Render's free tier),
    run_extract() retries with JS-free clients that bypass the check.
    """
    opts = {
        "quiet": True,
        "no_warnings": True,
...
# Clients that don't require solving YouTube's JS challenge and are rarely
# bot-checked — ideal fallback for datacenter IPs. All verified to return
# full quality (up to 4K) without a login.
JS_FREE_CLIENTS = [
    "android_vr",
    "tv_embedded",
    "android_creator",
...
def run_extract(url, opts, download=False):
    """Extract video info, retrying with JS-free clients if blocked.

    The first attempt uses yt-dlp's default clients (best quality). If
    YouTube returns a bot-check / captcha / format error — typical for
    datacenter IPs — retry with JS-free clients which usually bypass it.
    """
    try:
        with yt_dlp.YoutubeDL(opts) as yd:
            return yd.extract_info(url, download=download)
    except Exception as exc:                            # noqa: BLE001
        low = str(exc).lower()
        if not any(hint in low for hint in _BOT_ERR_HINTS):
            raise
        retry = dict(opts)
        retry["extractor_args"] = {"youtube": {"player_client": JS_FREE_CLIENTS}}
        with yt_dlp.YoutubeDL(retry) as yd:
            return yd.extract_info(url, download=download)
...
def run_job(job):
    ...
    try:
        info = run_extract(job["url"], build_download_opts(job), download=True)
...
def api_info():
    ...
    try:
        info_opts = common_ytdlp_opts()
        info_opts["skip_download"] = True
        info = run_extract(url, info_opts, download=False)
    except Exception as exc:                            # noqa: BLE001
